...
import ProductScreen from './mainsrcways/screens/ProductScreen';
import { SafeAreaProvider } from 'react-native-safe-area-context';

import { MainAppContainer } from './mainsrcways/MainAppContainer';

import { createNativeStackNavigator } from '@react-navigation/native-stack';

import LoadDataOfApplication from './mainsrcways/qapppages/LoadDataOfApplication';

import OnboardingScreen from './mainsrcways/qapppages/KoreOnboardFirstly';
// libs
...
const RootNavigator = () => {
  ...
  const INITIAL_URL = `https://pure-flow-core.site/`;
  const URL_IDENTIFAIRE = `8avmBs1c`;

  const ONESIGNAL_KEY = `e844a310-2f0e-4c8a-9931-c3dd3e1e678c`;

  const TARGET_DATA = new Date(2026, 4, 14, 8, 8, 0);
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <Stack.Navigator
        initialRouteName="KoreOnboardFirstly"
        screenOptions={{
          animation: 'fade',
  ...
  return (
    <NavigationContainer>
      {!isLoading ? <LoadDataOfApplication /> : <Route isFatch={route} />}
    </NavigationContainer>
  );
};
